...
def message_you_sent_end_command_earlier_and_timer_expired() -> str:
    return "Ваше терпение вознаграждено: вы просили сменить собеседника, вот только у второй стороны, кажется, были свои взгляды на это. Но мучение не должно длиться вечно, можете попробовать пообщаться ещё с кем-нибудь ('/start_chatting' вам в помощь)."
...
def message_you_have_reached_the_next_tier(
    ...
    return f"Ну что ж, собеседник оценил ваши ответы суммарно на {current_score} баллов. Теперь вам открывается новый уровень доступа к профилю вашего собеседника, используйте разумно эту возможность (№ {reached_tier})."
...
def message_you_cannot_unregister_now():
    return "Удалить регистрацию возможно только тогда, когда вы не находитесь в процессе заполнения профиля, общения с собеседником или инициации прекращения беседы. Пожалуйста, доделайте то, что собирались сделать, и уже тогда снова используйте '/unregister', чтобы удалить свой профиль. А ещё такое может быть, если вы пытаетесь удалить регистрацию, которой у вас просто нет. В таком случае — используйте команды '/start' и '/register'."
...
def message_you_cannot_register_now_please_unregister():
    return "Вы не можете зарегистрироваться сейчас, простите. Возможно, если вы используете команду '/unregister', у вас получится. Но никак не ранее этого. Или попробуйте '/start', возможно вы ещё не сделали этого."

def message_your_next_please_command_received_successfully_now_wait():
    return "Ваш запрос на смену собеседника дошёл до бота. А теперь — ждите: или пока собеседник отправит такой же запрос, или пока таймаут истечёт"
...
def message_below_all_the_text_is_from_chat_partner(): 
    return "ВНИМАНИЕ!\n\nДо окончания беседы сообщения, не помеченные значком бота (🤖), принадлежат вашему собеседнику. Учитывайте это."

def message_your_next_tier_was_hown_to_the_partner(tier:int):
    return f"Вы поставили достаточное количество лайков, чтобы собеседнику было показан новый уровень вашего профиля (№ {tier})."
